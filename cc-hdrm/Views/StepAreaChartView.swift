...
struct StepAreaChartView: View {
    ...
    static func makeChartPoints(from polls: [UsagePoll]) -> [ChartPoint] {
        ...
        let gapThreshold = SparklinePathBuilder.gapThresholdMs
...
private struct HoverOverlayContent: View {
    ...
    private func tooltipView(for point: StepAreaChartView.ChartPoint) -> some View {
        ...
        VStack(alignment: .leading, spacing: 2) {
            ...
            if point.extraUsageActive == true,
               let usedCredits = point.extraUsageUsedCredits,
               let monthlyLimit = point.extraUsageMonthlyLimit {
                HStack(spacing: 3) {
                    Circle().fill(Color.extraUsageCool).frame(width: 6, height: 6)
                    Text("Extra: \(AppState.formatCents(Int(usedCredits.rounded()))) of \(AppState.formatCents(Int(monthlyLimit.rounded())))")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
