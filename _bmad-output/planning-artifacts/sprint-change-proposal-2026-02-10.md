...
| **Epic 16** (new)         | **Create**                     | "Subscription Intelligence" — 6 stories covering brainstorming Themes 2–5. Theme 1 (denominator fix) acknowledged as pre-completed in Story 14.3.                   |
...
- **No schema changes** — all new analysis is computable from existing `reset_events` and `usage_polls` tables
...
- **Effort:** Medium — rewriting 2 stories is small; planning and implementing 6 new stories is the main work
- **Risk:** Low — no completed work reverted, new stories are additive, all data sources already exist
...
SubscriptionPatternDetector analyzes ResetEvent history for 4 patterns:

- Forgotten subscription: <5% utilization for 2+ consecutive weeks
- Chronic overpaying: usage fits cheaper tier for 3+ consecutive months
- Chronic underpowering: rate-limited N+ times per cycle for 2+ cycles
- Usage decay: monthly utilization declining for 3+ consecutive months

Returns typed findings array; empty when no patterns detected; skips patterns with insufficient history.
...
TierRecommendationService compares actual usage against all tiers:

- Returns .downgrade, .upgrade, or .goodFit recommendation
- 20% safety margin above actual peak (configurable)
- Includes dollar figures (current cost, recommended cost, savings/additional cost)
...
Requires backlog reorganization (rewrite 2 stories, add 1 epic with 6 stories) plus artifact updates.
...
| **Product Manager (John)** | Update PRD: fix FR40 terminology, add new FRs for Themes 2–5. Can be bundled into Create Story workflow.                                                                                      |
| **UX Designer (Sally)**    | Update UX Phase 3 Feature 4 spec. Recommended before Epic 16 stories begin.                                                                                                                   |
| **Architect (Winston)**    | Update architecture doc with new service descriptions (SubscriptionPatternDetector, TierRecommendationService). Can be done during Create Story.                                              |
...
- Slow-burn patterns detected and surfaced via macOS notifications
- Tier recommendations computed and displayed when actionable
- Billing cycle preference available in settings
- (Future) Insight engine selects most relevant conclusion
