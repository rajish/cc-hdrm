...
| Threshold prefs changed | prefs change while scoped machine is warned | Same rules as 5h/7d: re-arm only when headroom ≥ new warning (e.g. warning 20→10 at 15% headroom re-arms; 20→30 at 25% does not — state stays `warned20`, no fire); from `aboveWarning`, a raise that puts headroom below the new warning fires immediately | No error |
...
- [x] [Review][Patch] I/O matrix row "Threshold prefs changed" states a self-contradictory illustrative outcome; corrected to the implemented 5h/7d-parity behavior [_bmad-output/implementation-artifacts/spec-21-4-fable-threshold-notifications.md]
- [x] [Review][Patch] Auto Run Result "Files changed" list omitted the regenerated `epic-21-context.md`; added [_bmad-output/implementation-artifacts/spec-21-4-fable-threshold-notifications.md]
- [x] [Review][Patch] Two pre-existing vacuous absolute-time assertions `contains("at ")` in the 5h/7d body tests tightened to `contains("(at ")` [cc-hdrmTests/Services/ThresholdStateMachineTests.swift:247]
- [x] [Review][Defer] Threshold dedup state (5h, 7d, and now scoped) survives sign-out; a different account signing in inherits the previous account's warned state [cc-hdrm/App/AppDelegate.swift:373] — deferred, pre-existing
...
- `_bmad-output/implementation-artifacts/epic-21-context.md` — regenerated by the workflow (stale cache: three planning docs were newer); rides in this diff.
